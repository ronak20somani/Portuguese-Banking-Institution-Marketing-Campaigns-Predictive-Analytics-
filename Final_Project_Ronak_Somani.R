...
#EDA11 - Observing whether clients having housing loans subscribe for term deposit or not.
bank %>% ggplot(aes(x=housing, fill=y)) +
  geom_bar(position = 'dodge') 

#EDA12 - Observing whether clients having balance loans subscribe for term deposit or not.
bank %>% ggplot(aes(x=log1p(balance), fill=y)) +
  geom_density(alpha=0.5) 
